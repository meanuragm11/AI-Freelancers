...
export type ServiceFormWizardProps = {
  builderId: string;
  ...
  onClose?: () => void;
  onSaved?: () => void;
  onPublished?: (form: ServiceFormState) => void;
...
export default function ServiceFormWizard({
  ...
  const persistDraft = async (formSnapshot: ServiceFormState, closeAfterSave: boolean) => {
    ...
    const validation = validateServiceForm(prepared, "draft");
    if (!validation.valid) {
    ...
    setErrors([]);
    try {
      const saved = await saveServiceFromForm(builderId, prepared, {
  ...
  const handleSave = async (publish: boolean) => {
    const formSnapshot = prepareFormSnapshot(step === 1 ? buildFormWithCommittedChips() : form);
  ...
  const footer = (
    <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 md:flex-row md:items-center md:justify-between">
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          disabled={!embedded && step === 1 && !onExitFirstStep}
          onClick={handleBack}
      ...
      <div className="flex flex-wrap gap-2 md:justify-end">
        {showSaveAndNext && (
          <button
            type="button"
            disabled={(!embedded && isLastStep) || saving || uploading}
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">{stepContent}</div>
      {footer}
    </>
  );
}