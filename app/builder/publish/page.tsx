...
export default function PublishTool() {
  ...
  const [loadingType, setLoadingType] = useState<'dashboard' | 'live' | null>(null);
  ...
  // Unified publishing engine that handles both standard publishing and live preview routing
  const handlePublish = async (action: 'dashboard' | 'live') => {
    setLoading(true);
    ...
    if (user) {
      ...
      // The Magic Addition: .select().single() returns the generated row data (including the ID)
      ...
      if (!error && newListing) {
        if (action === 'live') {
          // Route directly to the real buyer page using the new ID
          router.push(`/buyer/tool/${newListing.id}`);
        } else {
          // Route back to the builder dashboard
    ...
    <div className="min-h-screen bg-slate-50 font-sans relative">
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Draft New Tool</h1>
          </div>
        </div>
        <div className="flex items-center gap-3">
          
          {/* Action 1: Publish & Return to Dashboard */}
          <button 
            onClick={() => handlePublish('dashboard')}
            disabled={loading || !formData.title || !formData.price}
            className="flex items-center gap-2 bg-slate-100 text-slate-700 px-5 py-2.5 rounded-xl text-xs font-bold tracking-widest hover:bg-slate-200 transition-colors uppercase border border-slate-200 disabled:opacity-50"
          >
            {loading && loadingType === 'dashboard' ? 'Saving...' : 'Save & Exit'}
          </button>
          
          {/* Action 2: Publish & Teleport to Live Page */}
          <button 
          ...
          >
            {loading && loadingType === 'live' ? 'Publishing...' : <><EyeIcon /> Publish & View Live</>}
          </button>

        </div>
      </div>

      {/* ── Main Form (Centered Document Style) ── */}
        ...
        <div className="bg-white p-8 md:p-12 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-8">
          ...
          <div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2 block">Value Proposition (Short Description)</label>
                <textarea 
                  name="description" value={formData.description} onChange={handleChange} rows={3}
                  placeholder="What business problem does this solve in one sentence?"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Slack, Salesforce, Postgres"
                    className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none transition-all"
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    placeholder="OCR, Legal, Dashboard"
                    className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 outline-none transition-all"
                  />
                </div>
              </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mt-2">
            <p className="text-xs font-bold text-blue-800 uppercase tracking-widest mb-1">Verification Process</p>
            <p className="text-xs text-blue-700/80 font-medium">Your tool will be published immediately without a verified badge. Our team will review your identity and demo URL within 24 hours to award the blue checkmark.</p>
          </div>
