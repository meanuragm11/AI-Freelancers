...
export default function GeographicWallet() {
  ...
  const handleRazorpayConnect = async () => {
    setLoading(true);
    // Future API call: /api/razorpay/connect
    alert("Initiating Razorpay Route Onboarding for Indian Bank Account...");
    setLoading(false);
  };

  const handleInternationalSave = async () => {
    setLoading(true);
    // Save PayPal email to user's profile in Supabase
    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-[#0f172a] border border-slate-800 rounded-3xl p-8 relative overflow-hidden shadow-xl">
          ...
          <h2 className="text-5xl font-black text-white tracking-tight mb-4">₹0.00</h2>
          
          <p className="text-sm text-slate-400 font-medium mb-6">
            Cleared funds from completed platform bounties.
          </p>

          <button disabled className="w-full bg-slate-800 text-slate-500 py-3 rounded-xl text-xs font-black uppercase tracking-widest cursor-not-allowed border border-slate-700">
            Withdraw Funds
          </button>
        </div>

        {/* The Geo-Router Setup */}
            ...
            <div className="mt-auto animate-in fade-in slide-in-from-bottom-4">
              <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl mb-4">
                <p className="text-xs text-blue-400 font-medium">
                  Indian residents receive automated bank transfers via Razorpay Route. No withdrawal fees.
                </p>
              </div>
              <button 
              ...
              >
                {loading ? 'Routing...' : 'Connect Indian Bank'}
              </button>
            </div>
          )}
            ...
            <div className="mt-auto animate-in fade-in slide-in-from-bottom-4">
              <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl mb-4">
                <p className="text-xs text-amber-400 font-medium">
                  International payouts are processed via PayPal on the 1st and 15th of every month. Standard PayPal cross-border fees apply.
                </p>
              </div>
              <input 
                type="email" 
              ...
              >
                {loading ? 'Saving...' : 'Save International Details'}
              </button>
            </div>
          )}
